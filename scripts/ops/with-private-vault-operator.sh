...
[ "$caller" = 'arn:aws:iam::106760547719:user/jsyang' ] || fail 'current AWS identity is not the reviewed Vault operator'
...
login="$(VAULT_TOKEN='' vault login -method=aws -path=operator-aws -no-store -format=json \
  ...
  header_value=node-operator-vault-operator-106760547719-apne2 2>/dev/null)" || fail 'Vault AWS operator login failed'
