...
cleanup() { set +e; unset VAULT_TOKEN; unlink "$tmp/onboarding.hcl" "$tmp/runtime.hcl" "$tmp/slashing-db.hcl" "$tmp/role.json" "$tmp/slashing-db-role.json" 2>/dev/null || true; rmdir "$tmp" 2>/dev/null || true; }
