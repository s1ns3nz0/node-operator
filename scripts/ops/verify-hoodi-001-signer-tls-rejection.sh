...
# Fixed to the reviewed hoodi-001 signer UID; never accepts real key material.
set -euo pipefail
umask 077
...
trap cleanup EXIT
kubectl -n "$namespace" get statefulset/validator-hoodi-001-client deployment/validator-hoodi-001-signing-fence -o json | jq -e '(.items|length)==2 and all(.items[];.spec.replicas==0)' >/dev/null
kubectl -n "$namespace" get pods -o json | jq -e 'all(.items[]; .metadata.labels["node-operator.io/validator-set"]!="hoodi-001" or (.metadata.labels["app.kubernetes.io/component"]!="validator-client" and .metadata.labels["app.kubernetes.io/component"]!="validator-signing-fence"))' >/dev/null
kubectl -n "$namespace" get lease validator-hoodi-001-primary -o json | jq -e '(.spec.holderIdentity//"")==""' >/dev/null
kubectl -n "$namespace" get deployment validator-hoodi-001-remote-signer -o json | jq -e '.metadata.uid=="d173bc2b-7e77-4864-84c9-9074c853c700" and .spec.replicas==1 and .status.readyReplicas==1 and .status.observedGeneration==.metadata.generation' >/dev/null
...
python3 "$repo/scripts/ops/render-signer-tls-probes.py" --validator-set hoodi-001 --expected-public-key 0xa3866b82651039224bfd725fc81e7ff17c1765021dff37f3fd4bc01405e2ed14c97c7c5c82cd95104d8f82c4229ab0d0 --fixture-name "$fixture" > "$evidence_dir/probes.json"
...
kubectl -n "$namespace" get deployment validator-hoodi-001-remote-signer -o json | jq -e '.spec.replicas==1 and .status.readyReplicas==1' >/dev/null
cleanup
trap - EXIT
for role in pre badca badclient post; do
 test -z "$(kubectl -n "$namespace" get pod "signer-tls-$role-hoodi-001" --ignore-not-found -o name)"
done
