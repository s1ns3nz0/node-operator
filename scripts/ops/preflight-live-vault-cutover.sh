...
usage() { printf 'Usage: %s --validator-set <hoodi-id> --evidence-output <new-absolute-json> [--phase baseline|ready]\n' "${0##*/}" >&2; exit 64; }
...
case "$phase" in baseline|ready) ;; *) usage;; esac
...
for target in \
  'node-operator|engine-api-jwt|jwt' \
  "validator-operations|validator-${validator_set}-signer-tls|tls-password.txt,tls.p12" \
  "validator-operations|validator-${validator_set}-client-tls|ca.crt,tls.crt,tls.key" \
  'validator-operations|vault-agent-ca|ca.crt'; do
  IFS='|' read -r namespace name expected <<<"$target"
...
mkdir -p "$(dirname "$evidence")"
  ...
  '{schema_version:1,operation:"live-vault-cutover-preflight",phase:$phase,validator_set:$set,collected_at_utc:$collected,argocd_revision:$revision,vault_active:true,vault_injector_ready:true,engine_pair_ready:true,validator_signer_ready:true,validator_client_ready:true,engine_vault_egress_ready:$vault_egress,source_secret_key_inventory_only:true,secret_values_emitted:false}' > "$evidence"
chmod 600 "$evidence"
