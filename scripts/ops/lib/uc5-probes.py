...
TLS_IMAGE = "106760547719.dkr.ecr.ap-northeast-2.amazonaws.com/node-operator-baseline-validator-signer-identity-probe@sha256:11212189c98afaeb719eaaa8ac7d3c86179884c84986c97c9c1a35b6b4f4ec8f"
TCP_IMAGE = "106760547719.dkr.ecr.ap-northeast-2.amazonaws.com/node-operator-baseline-node-runtime-upcheck-python@sha256:fb305627ba331d70f8cc82509a4fae4be858422fd257e170543fbbefcefb9588"
...
mode=sys.argv[1]
host="validator-hoodi-001-remote-signer-direct.validator-operations.svc"
try:
...
def tcp_manifest(mode, run):
    ...
    return {"apiVersion": "v1", "kind": "Pod",
            "metadata": {"name": "uc5-tcp-" + mode + "-" + run, "namespace": NS,
                         "labels": {"app.kubernetes.io/component": component, "node-operator.io/validator-set": "hoodi-001"},
...
class ProbeService:
    ...
    def new_signer_identity(self, baseline, delete_started_at):
        ...
        def observe():
            ...
            deployment = self._get("deployment", "validator-hoodi-001-remote-signer")
            ...
            pods = self.runner(["-n", NS, "get", "pods", "-l",
                                "app.kubernetes.io/component=validator-remote-signer,node-operator.io/validator-set=hoodi-001", "-o", "json"])["items"]
    ...
    def _tls(self, run):
        p = RENDER.tls("prepositive", "hoodi-001", self.key, TLS_IMAGE, run)
    ...
    def fresh_fence(self, baseline):
        ...
        pods = self.runner(["-n", NS, "get", "pods", "-l", "app.kubernetes.io/component=validator-signing-fence,node-operator.io/validator-set=hoodi-001", "-o", "json"])["items"]
        ...
        proof = {"schema_version": 1, "event_type": "signing-proxy-fence", "collected_at_utc": utc(),
                 "network": "hoodi", "validator_set": "hoodi-001", "validator_public_key": self.key,
    ...
    def history(self, phase):
        ...
        rows = self.runner(["-n", NS, "exec", "validator-hoodi-001-slashing-db-0", "-c", "postgres", "--", "psql", "--no-psqlrc", "--quiet", "--username=web3signer", "--dbname=web3signer", "--no-password", "--tuples-only", "--no-align", "--set=ON_ERROR_STOP=1", "--command", sql])
    ...
    def confirm_denied_start(self, pod_identity, delete_started_at):
        context = {"runtime_role_path": "auth/kubernetes/role/hoodi-hoodi-001-runtime",
                   "role_hmac": self.audit_hmac_reader(),
