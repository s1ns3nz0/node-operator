...
jq -e --arg name "$signer" --arg ns "$namespace" --arg set "$set_name" '
  ...
  (.spec.template.spec.containers[0].image | test("^106760547719\\.dkr\\.ecr\\.ap-northeast-2\\.amazonaws\\.com/node-operator-baseline-validator-runtime-web3signer@sha256:[a-f0-9]{64}$")) and
