...
usage() { printf '%s\n' "Usage: ${0##*/} --validator-set <hoodi-id> --validator-public-key <0x-key> --prysm-validator-image <private-ecr@sha256> --signing-fence-image <private-ecr@sha256> --kubernetes-api-cidr <ipv4/32> --output <absolute-yaml>" >&2; exit 64; }
validator_set=''; public_key=''; image=''; fence_image=''; kubernetes_api_cidr=''; output=''
while [ "$#" -gt 0 ]; do case "$1" in --validator-set) validator_set="${2:-}"; shift 2 ;; --validator-public-key) public_key="${2:-}"; shift 2 ;; --prysm-validator-image) image="${2:-}"; shift 2 ;; --signing-fence-image) fence_image="${2:-}"; shift 2 ;; --kubernetes-api-cidr) kubernetes_api_cidr="${2:-}"; shift 2 ;; --output) output="${2:-}"; shift 2 ;; *) usage ;; esac; done
...
printf '%s\n' "$fence_image" | grep -Eq '^106760547719\.dkr\.ecr\.ap-northeast-2\.amazonaws\.com/node-operator-baseline-validator-fence@sha256:[a-f0-9]{64}$' || { printf '%s\n' 'fence image must be the protected type-separated private ECR digest' >&2; exit 65; }
...
jq -e --arg image "$image" '.schema_version == 2 and any(.images[]; .private_image == $image and .stage_approved == true and (.release_channel == "upstream-mirror" or .release_channel == "manual-native-mtls"))' "$root/.ci/validator/approved-client-images.json" >/dev/null || { printf '%s\n' 'private Prysm image is not an exact stage-approved reviewed artifact' >&2; exit 65; }
