...
usage() { printf '%s\n' "Usage: ${0##*/} --validator-set <hoodi-id> --validator-public-key <0x-key> --aws-account-id <12-digit-id> [--aws-region <ap-northeast-1|ap-northeast-2>] --prysm-validator-image <private-ecr@sha256> --signing-fence-image <private-ecr@sha256> --kubernetes-api-cidr <ipv4/32> --output <absolute-yaml>" >&2; exit 64; }
validator_set=''; public_key=''; aws_account_id=''; aws_region='ap-northeast-2'; image=''; fence_image=''; kubernetes_api_cidr=''; output=''
while [ "$#" -gt 0 ]; do case "$1" in --validator-set) validator_set="${2:-}"; shift 2 ;; --validator-public-key) public_key="${2:-}"; shift 2 ;; --aws-account-id) aws_account_id="${2:-}"; shift 2 ;; --aws-region) aws_region="${2:-}"; shift 2 ;; --prysm-validator-image) image="${2:-}"; shift 2 ;; --signing-fence-image) fence_image="${2:-}"; shift 2 ;; --kubernetes-api-cidr) kubernetes_api_cidr="${2:-}"; shift 2 ;; --output) output="${2:-}"; shift 2 ;; *) usage ;; esac; done
