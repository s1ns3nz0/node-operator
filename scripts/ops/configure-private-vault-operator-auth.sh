...
usage() { printf 'usage: %s --principal-arn arn:aws:iam::106760547719:user/NAME\n' "${0##*/}" >&2; exit 64; }
principal=''
...
[ -n "$principal" ] || usage
[[ "$principal" =~ ^arn:aws:iam::106760547719:user/[A-Za-z0-9+=,.@_-]+$ ]] || {
  printf '%s\n' 'principal must be one exact IAM user ARN in account 106760547719, without a path or wildcard' >&2; exit 64;
}
...
vault write auth/operator-aws/config/client sts_region=ap-northeast-2 sts_endpoint=https://sts.ap-northeast-2.amazonaws.com use_sts_region_from_client=false iam_server_id_header_value=node-operator-vault-operator-106760547719-apne2 >/dev/null 2>&1 || fail 'failed to configure regional STS and IAM server header'
...
jq -e '.data.sts_region == "ap-northeast-2" and .data.sts_endpoint == "https://sts.ap-northeast-2.amazonaws.com" and .data.use_sts_region_from_client == false and .data.iam_server_id_header_value == "node-operator-vault-operator-106760547719-apne2"' <<<"$config" >/dev/null || fail 'AWS auth client readback differs from required configuration'
