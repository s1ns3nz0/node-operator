...
if [ "$#" -ne 2 ] || [ "$1" != --principal-arn ] ||
   [[ ! "$2" =~ ^arn:aws:iam::106760547719:user/[A-Za-z0-9+=,.@_-]+$ ]]; then
...
decision="$(aws iam simulate-principal-policy \
  --policy-source-arn arn:aws:iam::106760547719:role/node-operator-baseline-vault \
...
login="$(VAULT_TOKEN='' vault login -method=aws -path=operator-aws -no-store -format=json \
  ...
  header_value=node-operator-vault-operator-106760547719-apne2 2>/dev/null)" || {
