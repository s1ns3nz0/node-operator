...
gh variable set AWS_ACCOUNT_ID --repo "$repository" --env "$environment" --body "$account"
gh variable set GITOPS_CLIENT_ECR_PUBLISHER_ROLE_ARN --repo "$repository" --env "$environment" --body "$role"
