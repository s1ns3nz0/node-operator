...
usage() {
  printf '%s\n' "usage: ${0##*/} --aws-account-id <12-digit-id> --output-dir <new-absolute-dir> [--name <dns-name>] [--backend-principal-arn <same-account-role-arn>]" >&2
  exit 64
}

account=''; output_dir=''; name='node-operator'; principals=()
while [ "$#" -gt 0 ]; do
  case "$1" in
...
jq -n --arg account "$account" --arg name "$name" --argjson principals "$(printf '%s\n' "${principals[@]}" | jq -R . | jq -s .)" \
  '{aws_account_id:$account,name:$name,state_bucket_name:null,backend_principal_arns:$principals}' > "$bootstrap"
jq -n --arg name "$name" '{aws_region:"ap-northeast-2",name:$name,network_mode:"fresh"}' > "$foundation"
jq -n --arg account "$account" --arg name "$name" '
  {aws_account_id:$account,name:$name,enable_gitops_client_ecr_publisher:true,
...
' > "$baseline"
jq -n --arg account "$account" --arg name "$name" --arg bootstrap "$bootstrap" --arg foundation "$foundation" --arg baseline "$baseline" \
  '{schema_version:1,aws_account_id:$account,name:$name,bootstrap_config:$bootstrap,foundation_config:$foundation,baseline_config:$baseline}' > "$metadata"
