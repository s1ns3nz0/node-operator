...
usage() {
  printf '%s\n' "Usage: ${0##*/} --validator-set <hoodi-id> --validator-public-key <0x-key> --withdrawal-address <0x-address> --aws-account-id <12-digit-id> --web3signer-image <private-ecr@sha256> --postgres-image <private-ecr@sha256> --prysm-validator-image <private-ecr@sha256> --signing-fence-image <private-ecr@sha256> --kubernetes-api-cidr <ipv4/32> --output-dir <new-absolute-dir>" >&2
  exit 64
}

validator_set=''; validator_public_key=''; withdrawal_address=''; aws_account_id=''; web3signer_image=''; postgres_image=''; prysm_image=''; fence_image=''; kubernetes_api_cidr=''; output_dir=''
while [ "$#" -gt 0 ]; do
  case "$1" in
...
"$root/scripts/ops/render-hoodi-validator-runtime.sh" --validator-set "$validator_set" --aws-account-id "$aws_account_id" --web3signer-image "$web3signer_image" --postgres-image "$postgres_image" --output "$runtime"
"$root/scripts/ops/render-hoodi-validator-client.sh" --validator-set "$validator_set" --validator-public-key "$validator_public_key" --aws-account-id "$aws_account_id" --prysm-validator-image "$prysm_image" --signing-fence-image "$fence_image" --kubernetes-api-cidr "$kubernetes_api_cidr" --output "$client"
