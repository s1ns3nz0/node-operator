...
def _expected(destination: Path, baseline: dict, artifact: dict, digest: str, discovery: dict) -> dict:
    ...
    if not (isinstance(kms, str) and _KMS.fullmatch(kms) and kms.startswith(f"arn:aws:kms:{region}:{account}:key/")
            ...
            and isinstance(repositories, dict) and set(repositories) >= {"vault", "vault_chart"}
            and all(isinstance(repositories[key], str) and re.fullmatch(rf"{account}\.dkr\.ecr\.{region}\.amazonaws\.com/[a-z0-9][a-z0-9._/-]*", repositories[key]) for key in ("vault", "vault_chart"))):
