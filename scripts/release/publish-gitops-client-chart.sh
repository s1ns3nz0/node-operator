...
gh workflow run publish-oci.yml --repo "$repository" --ref main
run_id="$(gh run list --repo "$repository" --workflow publish-oci.yml --branch main --limit 1 --json databaseId --jq '.[0].databaseId')"
[[ "$run_id" =~ ^[1-9][0-9]*$ ]] || { printf '%s\n' 'unable to resolve publisher run' >&2; exit 70; }
gh run watch "$run_id" --repo "$repository" --exit-status
artifact_id="$(gh api "repos/$repository/actions/runs/$run_id/artifacts?per_page=100" --jq '.artifacts[] | select(.name | startswith("gitops-chart-evidence-")) | select(.expired == false) | .id' | head -1)"
[[ "$artifact_id" =~ ^[1-9][0-9]*$ ]] || { printf '%s\n' 'publisher evidence artifact is unavailable' >&2; exit 70; }
tmp="$(mktemp -d)"; trap 'rm -rf "$tmp"' EXIT
gh api "repos/$repository/actions/artifacts/$artifact_id/zip" > "$tmp/evidence.zip"
