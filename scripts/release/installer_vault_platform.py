...
def run(bundle_root:Path,state_dir:Path,discovery:dict,profile:str)->dict:
 ...
 intent=state_dir/"vault-platform-intent.json"; build=state_dir/"vault-platform-build.json"; success=state_dir/"vault-platform-success.json"
 ...
 if status in {"FAILED","FAULT","STOPPED","TIMED_OUT"}:raise PlatformError("terminal build failure; reconcile without restart")
