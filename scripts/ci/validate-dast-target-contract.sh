...
jq -e '
  ...
  (.scanner | type == "object" and (keys | sort) == ["image", "profile"] and .profile == "baseline-passive" and (.image | test("^106760547719\\.dkr\\.ecr\\.ap-northeast-2\\.amazonaws\\.com/node-operator/[a-z0-9][a-z0-9._/-]*@sha256:[0-9a-f]{64}$"))) and
