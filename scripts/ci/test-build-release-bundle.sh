...
case "$authority_count" in
  0)
    prepare_validator "$prepared" \
      106760547719.dkr.ecr.ap-northeast-2.amazonaws.com/node-operator-baseline-validator-prysm@sha256:7fe554adf0efd27c0e5c5a3f80a3bbbec3d3872626208cf0a003b0dee7761f89 \
      106760547719.dkr.ecr.ap-northeast-2.amazonaws.com/node-operator-baseline-validator-fence@sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb >/dev/null
    ;;
  4)
    for authority_path in "${authority_paths[@]}"; do
    ...
    prepare_validator "$prepared" \
      "106760547719.dkr.ecr.ap-northeast-2.amazonaws.com/hoodi-release-001-baseline-validator-prysm@$prysm_digest" \
      "106760547719.dkr.ecr.ap-northeast-2.amazonaws.com/hoodi-release-001-baseline-validator-fence@$fence_digest" >/dev/null
    ;;
  *)
    set +e
    prepare_validator "$prepared" \
      106760547719.dkr.ecr.ap-northeast-2.amazonaws.com/node-operator-baseline-validator-prysm@sha256:7fe554adf0efd27c0e5c5a3f80a3bbbec3d3872626208cf0a003b0dee7761f89 \
      106760547719.dkr.ecr.ap-northeast-2.amazonaws.com/node-operator-baseline-validator-fence@sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb >"$temporary_directory/partial-validator.stdout" 2>"$temporary_directory/partial-validator.stderr"
    partial_status=$?
    set -e
