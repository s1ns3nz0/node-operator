...
jq -e --arg key "$key" '
  ...
  all(.items[]; .metadata.namespace == "validator-operations" and .metadata.annotations["vault.hashicorp.com/role"] == "hoodi-hoodi-001-client-tls" and .metadata.annotations["vault.hashicorp.com/agent-inject-secret-tls.crt"] == "node-operator-runtime/data/validators/hoodi/hoodi-001/runtime/client-tls" and (.metadata.annotations["vault.hashicorp.com/agent-inject-template-tls.key"] | contains("node-operator-runtime/data/validators/hoodi/hoodi-001/runtime/client-tls")) and .spec.restartPolicy == "Never" and .spec.activeDeadlineSeconds == 120 and .spec.automountServiceAccountToken == false and .spec.enableServiceLinks == false and .spec.serviceAccountName == "validator-client" and .spec.securityContext.runAsUser == 65532 and .spec.securityContext.runAsGroup == 65532 and .spec.securityContext.fsGroup == 65532 and .spec.securityContext.seccompProfile.type == "RuntimeDefault" and (.spec.containers | length == 1) and .spec.containers[0].name == "get-only-identity-probe" and .spec.containers[0].image == "106760547719.dkr.ecr.ap-northeast-2.amazonaws.com/node-operator-baseline-validator-signer-identity-probe@sha256:11212189c98afaeb719eaaa8ac7d3c86179884c84986c97c9c1a35b6b4f4ec8f" and .spec.containers[0].args == ["--validator-set","hoodi-001","--expected-public-key",$key] and .spec.containers[0].securityContext.readOnlyRootFilesystem == true and .spec.containers[0].securityContext.allowPrivilegeEscalation == false and .spec.containers[0].securityContext.capabilities.drop == ["ALL"] and .spec.containers[0].resources.requests.cpu == "10m" and .spec.containers[0].resources.limits.memory == "64Mi" and .spec.volumes[0].projected.sources[0].serviceAccountToken.audience == "vault" and .spec.volumes[0].projected.sources[0].serviceAccountToken.expirationSeconds == 600)
' "$output" >/dev/null
jq -e 'all(.items[];
