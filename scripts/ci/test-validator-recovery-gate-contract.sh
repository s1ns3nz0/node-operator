...
printf '%s\n' "$*" >> "$MOCK_TRACE"
case "$*" in
  *'get statefulset validator-hoodi-001-client '*)
    [ "${MOCK_API_FAIL:-false}" = false ] || exit 69
    printf '%s' "${MOCK_STATEFUL-0}" ;;
  *'get deployment validator-hoodi-001-client '*) printf '%s' "${MOCK_LEGACY-}" ;;
  *'get deployment validator-hoodi-001-signing-fence '*) printf '%s' "${MOCK_FENCE-0}" ;;
  *'get deployment validator-hoodi-001-remote-signer '*) printf '%s' "${MOCK_SIGNER-0}" ;;
  *'get pods '*)
  ...
  *'get pvc '*) printf '%s' "${MOCK_PVC_SET-hoodi-001}" ;;
  '-n validator-operations scale deployment validator-hoodi-001-remote-signer --replicas=1') ;;
  *) exit 64 ;;
esac
EOF
...
run_recovery() {
  PATH="$scratch/bin:$PATH" MOCK_TRACE="$scratch/trace" bash "$script" \
    --validator-set hoodi-001 --validator-public-key "$key" \
...
grep -Fxq -- '-n validator-operations scale deployment validator-hoodi-001-remote-signer --replicas=1' "$scratch/trace"
