...
class OpsExecutionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.output = {"schema_version": 1, "aws_region": "ap-northeast-1", "cluster_name": "test-node", "ssm_ops_instance_id": "i-0123abcd"}
        self.calls = []
    ...
    def test_apply_returns_only_exact_private_handoff(self):
        ...
        self.assertEqual(result["ssm_ops_instance_id"], "i-0123abcd")
    ...
    def test_unexpected_or_typed_bad_handoff_is_rejected(self):
        ...
        self.output = {"schema_version": True, "aws_region": "ap-northeast-1", "cluster_name": "test-node", "ssm_ops_instance_id": "i-0123abcd"}
        with self.patches():
            with self.assertRaises(execution.OpsExecutionError): execution.apply_ops_access(self.bundle, self.state, DISCOVERY, "operator", digest)
