...
printf '%s\n' '#!/usr/bin/env bash' 'case "$*" in *"pulls/1"*files*) printf "%s\n" "[{\"filename\":\"policy/decision.rego\"}]" ;; *"pulls/1"*reviews*) printf "%s\n" "[{\"state\":\"APPROVED\",\"commit_id\":\"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\",\"user\":{\"login\":\"fjybjinsu\"}}]" ;; *"pulls/1"*) printf "%s\n" "{\"user\":{\"login\":\"author\"},\"head\":{\"sha\":\"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\"}}" ;; *) exit 64 ;; esac' > "$temporary_directory/bin/gh"
...
PATH="$temporary_directory/bin:$PATH" GITHUB_EVENT_PATH="$temporary_directory/event.json" GITHUB_REPOSITORY="owner/repo" "$script_dir/collect-scm-posture.sh" aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa "$temporary_directory/workflow-run-scm.json" 1
