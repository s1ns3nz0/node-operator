...
jq -n --arg p "$inputs" '{schema_version:1,aws_account_id:"106760547719",bootstrap_config:($p+"/other.json"),foundation_config:($p+"/foundation-network.tfvars.json"),baseline_config:($p+"/baseline.tfvars.json")}' > "$inputs/zero-resource-inputs.json"
