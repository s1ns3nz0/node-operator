...
printf '%s\n' '#!/usr/bin/env bash' 'case " $* " in' '  *" sts get-caller-identity "*) printf "%s\n" 106760547719 ;;' '  *" eks describe-cluster "*) printf "%s\n" sg-0123456789abcdef0 ;;' '  *) exit 1 ;;' 'esac' > "$tools/aws"
chmod 700 "$tools/aws"
jq -n '{schema_version:"v1",aws_region:"ap-northeast-2",aws_account_id:"106760547719",cluster_name:"node-operator",vpc_id:"vpc-0123456789abcdef0",subnet_id:"subnet-0123456789abcdef0",backend:{bucket:"node-operator-tfstate-106760547719-apne2",dynamodb_table:"node-operator-terraform-lock",kms_key_id:"arn:aws:kms:ap-northeast-2:106760547719:key/01234567-89ab-cdef-0123-456789abcdef",region:"ap-northeast-2",key:"node-operator/ops-access/terraform.tfstate"}}' > "$handoff"
