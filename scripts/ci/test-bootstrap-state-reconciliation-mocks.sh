...
case "$AWS_SCENARIO:$service:$operation" in
  existing:s3api:list-buckets) printf '{"Buckets":[{"Name":"node-operator-tfstate-106760547719-apnortheast2"},{"Name":"node-operator-tfstate-106760547719-apnortheast2-01234567-logs"}]}' ;;
  ...
  existing:s3api:get-bucket-encryption) printf '{"ServerSideEncryptionConfiguration":{"Rules":[{"ApplyServerSideEncryptionByDefault":{"KMSMasterKeyID":"arn:aws:kms:ap-northeast-2:106760547719:key/key-123"}}]}}' ;;
...
run_helper() {
  AWS_CALL_LOG="$workspace/calls" AWS_SCENARIO="$1" PATH="$workspace/bin:$PATH" \
    python3 "$helper" --region ap-northeast-2 --deployment hoodi \
      --account-id 106760547719 --bucket node-operator-tfstate-106760547719-apnortheast2 \
      --logs-bucket node-operator-tfstate-106760547719-apnortheast2-01234567-logs \
...
: > "$workspace/calls"
run_helper existing \
  --state-address aws_s3_bucket.state --state-id aws_s3_bucket.state=node-operator-tfstate-106760547719-apnortheast2 \
  --state-address aws_s3_bucket.state_access_logs --state-id aws_s3_bucket.state_access_logs=node-operator-tfstate-106760547719-apnortheast2-01234567-logs \
