...
def node(name, os_name="linux", os_image="Amazon Linux 2023", architecture="amd64", provider="aws:///ap-northeast-2a/i-1234abcd", labels=None):
...
class EksCisJobsTests(unittest.TestCase):
    ...
    def test_rejects_fargate_windows_and_unsupported_nodes_instead_of_skipping(self):
        cases = [node("fargate", labels={"eks.amazonaws.com/compute-type": "fargate"}), node("windows", os_name="windows"), node("al2", os_image="Amazon Linux 2"), node("external", provider="kube://external"), node("wrong-region", provider="aws:///us-east-1a/i-1234abcd")]
        for item in cases:
