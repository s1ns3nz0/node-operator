...
for required in \
  ...
  'scale deployment "$fence_deployment" --replicas=1' \
...
printf '%s\n' 'PASS: validator cutover requires a fence proof, verifies Vault-only manifests, restores only signer/fence, and leaves client at zero.'
