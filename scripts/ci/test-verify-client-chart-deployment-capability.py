...
D='a'*64
VALUES={"deployment":{"profile":"deployment","storageKmsKeyId":"arn:aws:kms:us-east-1:106760547719:key/11111111-2222-3333-4444-555555555555"},"dast":{"enabled":False},"clients":{"vaultAgentImage":f"106760547719.dkr.ecr.us-east-1.amazonaws.com/node-op-2609140157-gitops-vault@sha256:{D}","deployment":{"nethermindImage":f"106760547719.dkr.ecr.us-east-1.amazonaws.com/node-op-2609140157-gitops-nodes@sha256:{D}","prysmImage":f"106760547719.dkr.ecr.us-east-1.amazonaws.com/node-op-2609140157-gitops-nodes@sha256:{D}","prysmP2PHostIp":"198.51.100.42"}}}
...
class T(unittest.TestCase):
 ...
 def test_lookup_binds_selected_profile_and_scrubs_ambient_tokens(self):
  ...
  with patch.dict(os.environ,{'AWS_PROFILE':'ambient','AWS_ACCESS_KEY_ID':'ambient-key','AWS_SECRET_ACCESS_KEY':'ambient-secret','AWS_SESSION_TOKEN':'ambient-token','AWS_WEB_IDENTITY_TOKEN_FILE':'/tmp/ambient-token','AWS_ENDPOINT_URL':'http://ambient.invalid'},clear=False):
   self.assertEqual(M.fetch_authorized_layer(f'106760547719.dkr.ecr.us-east-1.amazonaws.com/node-op-2609140157-baseline-gitops-client/node-operator-client@sha256:{digest}','selected-profile',runner,lambda *_args,**_kwargs:io.BytesIO(archive)),archive)
 ...
 def test_malformed_image_id_is_cleanly_rejected(self):
  ...
  with self.assertRaises(M.CapabilityError):M.fetch_authorized_layer(f'106760547719.dkr.ecr.us-east-1.amazonaws.com/node-op-2609140157-baseline-gitops-client/node-operator-client@sha256:{D}','selected-profile',runner)
