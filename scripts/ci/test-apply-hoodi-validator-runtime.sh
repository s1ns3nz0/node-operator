...
kind: NetworkPolicy
metadata:
  name: validator-hoodi-001-signing-fence-egress
...
spec:
  ...
  policyTypes: [Egress]
  egress:
    - to: [{podSelector: {matchLabels: {app.kubernetes.io/component: validator-remote-signer, node-operator.io/validator-set: hoodi-001}}}]
...
run_helper() { KUBECTL_TRACE="$scratch/trace" ENDPOINTS_JSON="$1" PATH="$tools:$PATH" PRIVATE_EKS_SESSION=1 "$helper" --runtime "$runtime" --client "$client" --rendered-client "$rendered" --beacon-service "$service" --validator-set hoodi-001; }
...
ruby -ryaml -e '
  ...
  abort "selector changed" unless p.dig("spec","podSelector","matchLabels")=={"app.kubernetes.io/component"=>"validator-signing-fence","node-operator.io/validator-set"=>"hoodi-001"}
