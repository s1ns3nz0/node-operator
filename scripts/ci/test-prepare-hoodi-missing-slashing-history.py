...
class PrepareTest(unittest.TestCase):
 def run_command(self, mode=""):
  with tempfile.TemporaryDirectory() as tmp:
   ...
   out=p/"receipt"; started=time.monotonic(); result=subprocess.run([str(COMMAND),"--validator-set","hoodi-001","--validator-public-key",KEY,"--exception-approval-id","approved-123","--output-dir",str(out),"--execute"],text=True,capture_output=True,env=env)
