...
#!/usr/bin/env bash
set -euo pipefail
if [ "$SCENARIO" = identity ]; then echo arn:aws:iam::106760547719:user/other; else echo arn:aws:iam::106760547719:user/jsyang; fi
MOCK
...
case "$1 $2" in
  'login -method=aws')
    ...
    [[ " $* " == *' header_value=node-operator-vault-operator-106760547719-apne2 '* ]]
    echo login >> "$CALLS"
