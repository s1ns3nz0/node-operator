...
digest="sha256:$(printf 'a%.0s' {1..64})"; revision="$(printf 'b%.0s' {1..40})"; image="106760547719.dkr.ecr.ap-northeast-2.amazonaws.com/node-operator-baseline-validator-fence@$digest"
statement="$(jq -cn --arg d "${digest#sha256:}" --arg r "$revision" '{_type:"https://in-toto.io/Statement/v0.1",predicateType:"https://slsa.dev/provenance/v1",subject:[{digest:{sha256:$d}}],predicate:{buildDefinition:{buildType:"https://slsa.dev/container-based-build/v1",resolvedDependencies:[{digest:{gitCommit:$r}}]},runDetails:{builder:{id:"https://github.com/Attestations/GitHubHostedActions@v1"}}}}' | base64 | tr -d '\n')"
...
wrong_revision_statement="$(jq -cn --arg d "${digest#sha256:}" '{_type:"https://in-toto.io/Statement/v0.1",predicateType:"https://slsa.dev/provenance/v1",subject:[{digest:{sha256:$d}}],predicate:{buildDefinition:{buildType:"https://slsa.dev/container-based-build/v1",resolvedDependencies:[{digest:{gitCommit:"0000000000000000000000000000000000000000"}}]},runDetails:{builder:{id:"https://github.com/Attestations/GitHubHostedActions@v1"}}}}' | base64 | tr -d '\n')"
