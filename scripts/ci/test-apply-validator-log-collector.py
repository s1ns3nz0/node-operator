...
class ApplyCollector(unittest.TestCase):
    ...
    def invoke(self, receipt=None):
        ...
        with patch.object(installer, "_module", load), patch.object(installer, "verify_full_mirror", return_value=receipt):
            installer.install(bundle_root=self.bundle, state_dir=self.state, work_dir=self.work, inputs_dir=self.inputs, session=self.state / "private-eks-session.json", baseline_config=self.inputs / "baseline.tfvars.json", account=ACCOUNT, region=REGION, deployment=DEPLOYMENT, validator_set="hoodi-001", profile="chosen", release_sha="a" * 40, runner=self.runner)
