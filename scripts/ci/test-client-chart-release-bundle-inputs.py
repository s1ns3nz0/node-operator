...
class T(unittest.TestCase):
 def setUp(self):
  ...
  auth={'schema_version':1,'source_revision':fixture_module.SOURCE,'publication':{'repository':'s1ns3nz0/node-operator-gitops','workflow':'publish-oci.yml','run_id':'1','artifact_id':'2','artifact_name':'gitops-chart-evidence-test','run_number':'37'},'target':{'image_ref':f'106760547719.dkr.ecr.ap-northeast-2.amazonaws.com/node-operator-baseline-gitops-client/node-operator-client@{digest}','manifest_digest':digest,'chart_archive_digest':archive,'chart_version':'0.1.37'},'evidence_sha256':{n:hashlib.sha256(b).hexdigest() for n,b in self.evidence.items()},'approvals':{'stage_approved':True,'activation_approved':False}}
