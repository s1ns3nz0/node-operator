...
case "$*" in
  '-n validator-operations get statefulset validator-hoodi-001-client -o json')
    ...
    workload StatefulSet validator-hoodi-001-client client-tls validator-hoodi-001-client-tls validator '["--validators-external-signer-http-client-cert=/vault/secrets/tls.crt","--validators-external-signer-http-client-key=/vault/secrets/tls.key","--validators-external-signer-http-ca-cert=/vault/secrets/ca.crt"]' "$replicas" ;;
  '-n validator-operations get deployment validator-hoodi-001-signing-fence -o json')
  ...
  '-n validator-operations get deployment validator-hoodi-001-remote-signer -o json')
    workload Deployment validator-hoodi-001-remote-signer signer-tls validator-hoodi-001-signer-tls web3signer '["--tls-keystore-file=/vault/secrets/tls.p12","--tls-keystore-password-file=/vault/secrets/tls-password.txt"]' 1 ;;
  *' patch deployment validator-hoodi-001-remote-signer --type=json -p '*|*' patch statefulset validator-hoodi-001-client --type=json -p '*)
    patch=''
    for ((i = 1; i <= $#; i++)); do
      if [ "${!i}" = -p ]; then
    ...
    [ -n "$patch" ] || exit 64
    if [[ "$*" == *' patch deployment '* ]]; then volume=signer-tls; secret=validator-hoodi-001-signer-tls; else volume=client-tls; secret=validator-hoodi-001-client-tls; fi
...
run() {
  MOCK_MODE="$1" MOCK_TRACE="$scratch/trace" PATH="$scratch/bin:$PATH" \
    bash "$script" --validator-set hoodi-001 --execute
}
