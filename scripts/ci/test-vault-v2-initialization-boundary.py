...
'''); remover.chmod(0o700)
    ...
    def invoke(self, data, directory, extra_args=(), **extra):
        return subprocess.run(["bash", str(self.script), "--validator-set", "hoodi-001", "--recovery-output-dir", str(directory), *extra_args], input=data, text=True, capture_output=True, env={**os.environ, "PATH": f"{self.bin}:{os.environ['PATH']}", "REAL_RM":self.real_rm, "PRIVATE_VAULT_SESSION":"1", "VAULT_ADDR":"https://vault.test", "STATE":str(self.state), "EVENTS":str(self.events), "CALLS":str(self.calls), "AUTH_ENABLED":str(self.auth_enabled), "AUTH_CONFIGURED":str(self.auth_configured), **extra})
    ...
    def test_completion_verification_never_replays_administrator_ceremony(self):
        ...
        checkpoint.write_text('{"schema_version":1,"status":"configured","cluster_id":"cluster-a","validator_set":"hoodi-001","shares":5,"threshold":3,"root_revoked":true}')
    ...
    def test_completion_verification_rejects_pending_or_mismatched_state_without_recovery(self):
        ...
        for case in ("missing", "backup-pending", "cluster", "revocation", "sealed"):
            ...
            value = {"schema_version":1,"status":"configured","cluster_id":"cluster-a","validator_set":"hoodi-001","shares":5,"threshold":3,"root_revoked":True}
...
''')
    ...
    def test_checkpoint_cluster_mismatch_rejects_without_recovery_ceremony(self):
        ...
        (output / "vault-initialization-checkpoint.json").write_text('{"schema_version":1,"status":"backup-pending","cluster_id":"other","validator_set":"hoodi-001","shares":5,"threshold":3}')
    ...
    def test_matching_pending_resume_configures_without_second_init(self):
        ...
        checkpoint = output / "vault-initialization-checkpoint.json"; checkpoint.write_text('{"schema_version":1,"status":"backup-pending","cluster_id":"cluster-a","validator_set":"hoodi-001","shares":5,"threshold":3,"raw_response":"vault-init-response.fixture"}'); checkpoint.chmod(0o600)
    ...
    def test_configured_checkpoint_rejects_cluster_root_revocation_or_unsafe_metadata(self):
        for kind, value in (("cluster", "other"), ("root", "cluster-a"), ("mode", "cluster-a")):
            with self.subTest(kind=kind):
                ...
                checkpoint.write_text('{"schema_version":1,"status":"configured","cluster_id":"' + value + '","validator_set":"hoodi-001","shares":5,"threshold":3,"root_revoked":' + revoked + '}')
