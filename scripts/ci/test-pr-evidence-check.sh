...
rg -Fx 'head_sha=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa' "$temporary_directory/pass.args" >/dev/null
rg -Fx 'conclusion=success' "$temporary_directory/pass.args" >/dev/null
rg -Fx 'CI Evidence Decision' "$temporary_directory/pass.args" >/dev/null
...
rg -Fx 'conclusion=failure' "$temporary_directory/fail.args" >/dev/null
rg -Fx 'output[summary]=Trusted OPA decision reported block=1 and require_approval=0.' "$temporary_directory/fail.args" >/dev/null
...
rg -Fx 'conclusion=failure' "$temporary_directory/malformed.args" >/dev/null
...
rg -Fx 'conclusion=failure' "$temporary_directory/upstream.args" >/dev/null
...
rg -Fx 'subject_sha=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa' "$temporary_directory/context.out" >/dev/null
rg -Fx 'trusted_sha=bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb' "$temporary_directory/context.out" >/dev/null
