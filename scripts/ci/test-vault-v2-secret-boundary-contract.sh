...
if ! PATH="$temporary/bin:$PATH" PRIVATE_VAULT_SESSION=1 VAULT_ADDR=https://vault.test AUTH_TRACE="$temporary/auth-trace" \
  bash "$temporary/ops/recover-and-onboard-hoodi-validator-keystore.sh" --validator-set hoodi-001 --keystore-dir "$temporary/keys" --signer-ca-output "$temporary/signer-ca.crt" --known-clients-output "$temporary/known-clients.txt" --refresh-auth-only <<< 'recovery-share' >/dev/null 2>&1; then
