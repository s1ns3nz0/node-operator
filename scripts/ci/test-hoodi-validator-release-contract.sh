...
expected = {
    ...
    "--account": "106760547719", "--region": "ap-northeast-2", "--profile": "chosen",
}
...
case "${1:-}:${2:-}" in
  sts:get-caller-identity) printf '106760547719\n' ;;
...
jq -n --arg baseline "$inputs/zero-resource/baseline.tfvars.json" '{schema_version:1,aws_account_id:"106760547719",aws_region:"ap-northeast-2",baseline_config:$baseline}' > "$inputs/zero-resource/zero-resource-inputs.json"
jq -n '{name:"node-op-001",aws_account_id:"106760547719",aws_region:"ap-northeast-2"}' > "$inputs/zero-resource/baseline.tfvars.json"
client_manifest="$scratch/validator-client.yaml"
printf '%s\n' 'cidr: 127.0.0.1/32' > "$client_manifest"
jq -n --arg client_manifest "$client_manifest" '{schema_version:1,network:"hoodi",aws_account_id:"106760547719",aws_region:"ap-northeast-2",validator_set:"hoodi-release-001",validator_public_key:("0x" + ("a" * 96)),client_manifest:$client_manifest,staged_client_replicas:0,staged_fence_replicas:0}' > "$inputs/validator-deployment/validator-deployment-handoff.json"
...
rg -F "inventory --bundle-root $bundle --release-sha aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa --aws-account-id 106760547719 --aws-region ap-northeast-2 --deployment-name node-op-001 --require-signer-probe profile=default" "$trace" >/dev/null
...
rg -F "mirror mirror --bundle-root $bundle --state-dir $scratch/work --work-dir $scratch/work --inputs-dir $inputs/zero-resource --account 106760547719 --region ap-northeast-2 --deployment-name node-op-001 --profile default --release-sha aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa profile=default" "$trace" >/dev/null
rg -F "mirror mirror --scope non-vault --bundle-root $bundle --state-dir $scratch/work --work-dir $scratch/work --inputs-dir $inputs/zero-resource --account 106760547719 --region ap-northeast-2 --deployment-name node-op-001 --profile default --release-sha aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa profile=default" "$trace" >/dev/null
...
jq '.ssm_ops_instance_id = "i-0abcdef1234567890"' "$original_custody_session" > "$deploy_session"
