...
case "$*" in
  ...
  'read -format=json auth/operator-aws/config/client') printf '%s\n' '{"data":{"sts_region":"ap-northeast-2","sts_endpoint":"https://sts.ap-northeast-2.amazonaws.com","use_sts_region_from_client":false,"iam_server_id_header_value":"node-operator-vault-operator-106760547719-apne2"}}' ;;
  'read -format=json auth/operator-aws/role/operator-recovery') printf '%s\n' '{"data":{"auth_type":"iam","bound_iam_principal_arn":["arn:aws:iam::106760547719:user/operator.recovery"],"bound_iam_principal_id":["AIDAEXACT"],"resolve_aws_unique_ids":true,"token_policies":["operator-recovery"],"token_ttl":300,"token_max_ttl":600,"token_explicit_max_ttl":600,"token_no_default_policy":true}}' ;;
  'policy read operator-recovery') cat "$POLICY" ;;
  'login -method=aws -path=operator-aws -no-store -format=json role=operator-recovery region=ap-northeast-2 header_value=node-operator-vault-operator-106760547719-apne2')
...
run() {
  ...
  set +e
  last_out="$(printf 'mock-share\n' | PATH="$tools:$PATH" TRACE="$scratch/$name.trace" POLICY="$policy" MODE="$mode" PRIVATE_VAULT_SESSION=1 "$wrapper" --principal-arn arn:aws:iam::106760547719:user/operator.recovery 2>&1)"
  rc=$?
  set -e
