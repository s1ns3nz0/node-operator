...
case "$*" in
  ...
  'read -format=json auth/operator-aws/config/client')
    if [ "${MODE:-ok}" = config-readback-bad ]; then
      printf '%s\n' '{"data":{"sts_region":"ap-northeast-2","sts_endpoint":"https://sts.amazonaws.com","use_sts_region_from_client":false,"iam_server_id_header_value":"node-operator-vault-operator-106760547719-apne2"}}'
    else
...
index=0
for bad in 'arn:aws:iam::106760547719:role/operator' 'arn:aws:iam::106760547719:user/path/name' 'arn:aws:iam::999999999999:user/operator' 'arn:aws:iam::106760547719:user/*'; do
