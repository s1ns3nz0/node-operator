...
def actual_custody_receipt_resume_contract():
    for tamper in (False, True):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            handoff.write_text(json.dumps({"validator_set":"hoodi-001", "validator_public_key":public_key})); handoff.chmod(0o600)
            input_file = inputs / "hoodi-zero-release-inputs.json"
            input_file.write_text(json.dumps({"aws_account_id":ACCOUNT,"aws_region":REGION,"validator_set":"hoodi-001","validator_deployment_handoff":str(handoff)})); input_file.chmod(0o600)
            ...
            result.write_text(json.dumps({"schema_version":1,"operation_id":prepared["operation_id"],"validator_set":"hoodi-001","expected_public_key":public_key,"result":"onboarding-complete","public_outputs":{"signer_ca_sha256":hashlib.sha256(signer.read_bytes()).hexdigest(),"known_clients_sha256":hashlib.sha256(known.read_bytes()).hexdigest()}})); result.chmod(0o600)
            ...
            if tamper:
                ...
                activation_receipt.write_text(json.dumps({
                    ...
                    "validator_set":"hoodi-001", "validator_public_key":public_key,
