...
s={'_type':'https://in-toto.io/Statement/v1','subject':[{'name':'106760547719.dkr.ecr.ap-northeast-2.amazonaws.com/node-operator-baseline-gitops-client/node-operator-client','digest':{'sha256':digest[7:]}}],'predicateType':'https://slsa.dev/provenance/v1','predicate':p}
...
auth={'schema_version':1,'source_revision':revision,'publication':{'repository':c.REPOSITORY,'workflow':c.WORKFLOW,'run_id':'1','artifact_id':'2','artifact_name':'gitops-chart-evidence-test','run_number':'37'},'target':{'image_ref':f'106760547719.dkr.ecr.ap-northeast-2.amazonaws.com/node-operator-baseline-gitops-client/node-operator-client@{digest}','manifest_digest':digest,'chart_archive_digest':archive,'chart_version':'0.1.37'},'evidence_sha256':{n:hashlib.sha256(b).hexdigest() for n,b in raw.items()},'approvals':{'stage_approved':True,'activation_approved':False}}
