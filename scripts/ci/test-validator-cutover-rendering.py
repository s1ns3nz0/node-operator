    ...
    def test_deployment_identity_must_match_protected_fence_prefix(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            subprocess.run(["bash", str(ROOT / "scripts/ops/render-hoodi-validator-client.sh"), *base,
                            ...
                            "--kubernetes-api-cidr", "172.20.0.1/32", "--output", str(client)],
                           check=True, capture_output=True)
            verifier.validate(runtime.read_text(), client.read_text(), "hoodi-001", self.key, self.images)
            with self.assertRaises(ValueError):
            ...
            verifier.validate(alternate_runtime, alternate_client, "hoodi-001", self.key, alternate_images)
            with self.assertRaises(ValueError):
