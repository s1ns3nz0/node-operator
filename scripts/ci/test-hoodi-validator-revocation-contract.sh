...
case "$*" in
  ...
  '-n validator-operations get deployment validator-hoodi-001-client --ignore-not-found -o jsonpath={.spec.replicas}') printf '%s' 0 ;;
  '-n validator-operations get pods -l app.kubernetes.io/component=validator-client,node-operator.io/validator-set=hoodi-001 -o name') printf '%s' "${MOCK_CLIENT_PODS:-}" ;;
...
run_probe() {
  PATH="$scratch/bin:$PATH" PRIVATE_VAULT_SESSION=1 \
    bash "$script" --validator-set hoodi-001 --exercise-approval-id reviewed-uc5 \
...
if MOCK_CLIENT_PODS='pod/validator-hoodi-001-client' run_probe "$scratch/proof.json" >/dev/null 2>&1; then
