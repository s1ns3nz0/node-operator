...
suite="${1:-all}"
[ "$#" -le 1 ] || exit 64
case "$suite" in all|root|runtime|modules|foundation|monitoring) ;; *) printf 'unknown Terraform suite\n' >&2; exit 64 ;; esac
...
if [ "$suite" = all ]; then
  for item in root runtime modules foundation monitoring; do run_suite "$item"; done
