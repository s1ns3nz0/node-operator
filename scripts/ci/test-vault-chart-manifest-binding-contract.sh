...
for required in \
  'variable "vault_chart_manifest_digest"' '^sha256:[a-f0-9]{64}$' \
  'VAULT_CHART_MANIFEST_DIGEST' 'ecr:DescribeImages' \
  'aws_ecr_repository.private_gitops["vault_chart"].arn' 'aws ecr describe-images' \
