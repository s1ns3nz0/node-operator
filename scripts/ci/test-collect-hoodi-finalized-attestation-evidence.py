...
KEY = "0x" + "a" * 96
IDENTITY = {"validator_set":"hoodi-001", "validator_public_key":KEY, "validator_index":"123", "deployment_name":"node-operator", "release_revision":"b" * 40}
...
class ArchiveCollection(unittest.TestCase):
    ...
    def event(self, identifier, timestamp, line, component="validator-remote-signer"):
        record = {"log":"2026-09-13T00:00:00Z stdout F " + line, "kubernetes":{"labels":{"node-operator.io/validator-set":"hoodi-001","node-operator.io/deployment-name":"node-operator","node-operator.io/release-revision":"b" * 40,"app.kubernetes.io/component":component}}}
    ...
    def fence(self, identifier="fence-1", opened=1950, closed=2050, timestamp=2050, result="closed", **overrides):
        fields = {"validator_set":"hoodi-001", "holder":"pod-uid-1", "lease":"validator-hoodi-001-primary", "connection_id":"0000000000000001", "opened_at_utc":"1970-01-01T00:00:01.95Z", "closed_at_utc":"1970-01-01T00:00:02.05Z", "opened_at_ms":str(opened), "closed_at_ms":str(closed), "result":result}
    ...
    def test_empty_archive_is_pending_but_api_denial_is_unavailable(self):
        ...
        arguments = ["--assignment-proof", str(self.assignment), "--archive-bucket", "node-audit-123", "--aws-region", "ap-northeast-2", "--validator-set", "hoodi-001", "--validator-public-key", KEY, "--validator-index", "123", "--deployment-name", "node-operator", "--release-revision", "b" * 40, "--activation-slot", "0", "--workload-output", str(output), "--log-delivery-output", str(delivery), "--archive-cursor", str(cursor_path)]
        stderr = io.StringIO()
