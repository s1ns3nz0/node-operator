...
expected_vault_bootstrap_input_sha="$({ sha256sum "$root/.ci/toolchains/vault-bootstrap.Dockerfile" "$root/release/vault-values.yaml.example" "$root/release/cert-manager-values.yaml.example" "$root/scripts/release/render-private-vault-values.sh" "$root/scripts/release/render-private-cert-manager-values.py" "$root/scripts/release/deploy-private-cert-manager.sh" "$root/scripts/release/deploy-sealed-vault.sh" "$root/scripts/release/prepare-vault-bootstrap-tls.sh" "$root/docs/gitops/vault-tls-internal-ca.example.yaml"; } | awk '{print $1}' | sha256sum | awk '{print $1}')"
output="$(cd "$root" && EXPECTED_INPUT_SHA="$expected_vault_bootstrap_input_sha" GITHUB_REPOSITORY=s1ns3nz0/node-operator GITHUB_SHA=fixture PATH="$temporary_directory/bin:$PATH" .ci/toolchains/release-toolchain-image.sh vault-bootstrap .ci/toolchains/vault-bootstrap.Dockerfile release/vault-values.yaml.example release/cert-manager-values.yaml.example scripts/release/render-private-vault-values.sh scripts/release/render-private-cert-manager-values.py scripts/release/deploy-private-cert-manager.sh scripts/release/deploy-sealed-vault.sh scripts/release/prepare-vault-bootstrap-tls.sh docs/gitops/vault-tls-internal-ca.example.yaml)"
