...
case "$*" in
  *'get statefulset validator-hoodi-001-client '*) [ "${MOCK_CLIENT_ABSENT:-false}" = true ] || jq -n --argjson r "${MOCK_CLIENT_REPLICAS:-0}" '{metadata:{name:"validator-hoodi-001-client"},spec:{replicas:$r,selector:{matchLabels:{"node-operator.io/validator-set":"hoodi-001"}},template:{metadata:{labels:{"node-operator.io/validator-set":"hoodi-001"}}}}}' ;;
  *'get deployment validator-hoodi-001-signing-fence '*) [ "${MOCK_FENCE_ABSENT:-false}" = true ] || jq -n --argjson r "${MOCK_FENCE_REPLICAS:-0}" '{metadata:{name:"validator-hoodi-001-signing-fence"},spec:{replicas:$r,selector:{matchLabels:{"node-operator.io/validator-set":"hoodi-001"}},template:{metadata:{labels:{"node-operator.io/validator-set":"hoodi-001"}}}}}' ;;
  *'get deployment validator-hoodi-001-remote-signer -o json') jq -n --argjson r "${MOCK_SIGNER_REPLICAS:-0}" '{metadata:{name:"validator-hoodi-001-remote-signer"},spec:{replicas:$r,selector:{matchLabels:{"node-operator.io/validator-set":"hoodi-001"}},template:{metadata:{labels:{"node-operator.io/validator-set":"hoodi-001"}}}}}' ;;
  *'get pods '*)
  ...
  *'get pvc data-validator-hoodi-001-slashing-db-0 -o json') jq -n --arg uid "${MOCK_PVC_UID:-pvc-uid}" --arg phase "${MOCK_PVC_PHASE:-Bound}" '{metadata:{name:"data-validator-hoodi-001-slashing-db-0",uid:$uid},spec:{resources:{requests:{storage:"50Gi"}}},status:{phase:$phase}}' ;;
  *'get lease validator-hoodi-001-primary -o json')
    if [ -f "$MOCK_STATE/patched" ]; then
      [ "${MOCK_READBACK_FAIL:-false}" = false ] || exit 1
      jq -n --arg uid "${MOCK_LEASE_UID:-lease-uid}" --arg renewed "$MOCK_RENEWED" --argjson duration "${MOCK_DURATION:-60}" '{metadata:{name:"validator-hoodi-001-primary",uid:$uid,resourceVersion:"rv-new"},spec:{holderIdentity:"",renewTime:$renewed,leaseDurationSeconds:$duration}}'
    else
      jq -n --arg uid "${MOCK_LEASE_UID:-lease-uid}" --arg rv "${MOCK_RV:-rv-old}" --arg holder "${MOCK_HOLDER:-old-holder}" --arg renewed "$MOCK_RENEWED" --argjson duration "${MOCK_DURATION:-60}" '{metadata:{name:"validator-hoodi-001-primary",uid:$uid,resourceVersion:$rv},spec:{holderIdentity:$holder,renewTime:$renewed,leaseDurationSeconds:$duration}}'
    fi ;;
  *'patch lease validator-hoodi-001-primary --type=json -p '*)
    patch="${!#}"
...
run_helper() {
  ...
  env PRIVATE_EKS_SESSION=1 PATH="$scratch/bin:$PATH" MOCK_TRACE="$scratch/trace" MOCK_STATE="$scratch/state" MOCK_RENEWED="$expired" "$@" "$script" --validator-set hoodi-001 --expected-lease-uid lease-uid --expected-resource-version rv-old --expected-holder old-holder --expected-pvc-uid pvc-uid --maintenance-approval-id reviewed-change-1 --output-dir "$out" ${dry_arg:+"$dry_arg"}
}
