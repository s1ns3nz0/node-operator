...
grep -Fq 'offchainlabs/prysm-validator@sha256:' <(workflow_source "$workflow")
grep -Fq 'docker buildx imagetools create' <(workflow_source "$workflow")
grep -Fq 'approved-client-images.json' <(workflow_source "$workflow")
grep -Fq '.mirror_eligible == true and .release_channel == "upstream-mirror" and .provenance_status == "upstream-release-mirror"' <(workflow_source "$workflow")
grep -Fq '::add-mask::' <(workflow_source "$workflow")
grep -Fq 'persist-credentials: false' "$workflow"
# This verifies literal workflow shell source.
# shellcheck disable=SC2016
grep -Fq 'test "$digest" = "${SOURCE_IMAGE#*@}"' <(workflow_source "$workflow")
jq -e '
