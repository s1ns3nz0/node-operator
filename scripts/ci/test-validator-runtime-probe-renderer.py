...
KEY = "0x" + "ab" * 48
IMG = (
    "106760547719.dkr.ecr.ap-northeast-2.amazonaws.com/node-operator-baseline-validator-signer-identity-probe@sha256:"
    + "a" * 64
)
...
class T(unittest.TestCase):
    def test_network_negative_has_no_engine_identity_or_credentials(self):
        ...
        p = module.engine_network_deny("hoodi-001", KEY, IMG, "test1")
        ...
        self.assertEqual(p["spec"]["containers"][0]["args"], ["--mode", "engine-network-deny", "--validator-set", "hoodi-001"])
    def test_engine_auth_secret_isolation(self):
        spec = importlib.util.spec_from_file_location("renderer", R)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        for case in ("prepositive", "postpositive", "missing", "wrong"):
            p = module.engine_auth(case, "hoodi-001", KEY, IMG, "test1")
    ...
    def test_engine_never_ready_placement(self):
        ...
        for role, sa, app in (("nethermind", "nethermind-execution", "nethermind"), ("prysm", "prysm-beacon", "prysm-beacon")):
            pod = module.engine_vault(role, "hoodi-001", KEY, IMG, "test1")
    ...
    def test_layout(self):
        ...
        self.assertNotIn("validator-hoodi-001-client-tls", json.dumps(tls))
