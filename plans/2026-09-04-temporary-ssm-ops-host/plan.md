...
4. Review a refresh-backed saved plan for only the host, its private security/IAM profile, and one-time termination scheduler resources before apply.
5. Verify private placement, zero inbound security-group rules, SSM management availability, and the exact termination schedule without opening a Kubernetes session.
