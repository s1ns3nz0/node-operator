...
func TestEngineJWTAndNegativeControls(t *testing.T) {
	...
	defer s.Close()
	if r, e := runEngineProbeWith(context.Background(), "hoodi-001", s.URL, f, "positive", now); e != nil || r["chain_id"] != "0x88bb0" {
		t.Fatalf("%v %#v", e, r)
	}
	for _, c := range []string{"missing", "wrong"} {
		if r, e := runEngineProbeWith(context.Background(), "hoodi-001", s.URL, "/does/not/exist", c, now); e != nil || r["result"] != "PASS" {
...
func TestEngineSafetyFailures(t *testing.T) {
	...
	for n, b := range cases {
		t.Run(n, func(t *testing.T) {
			...
			defer s.Close()
			if _, e := runEngineProbeWith(context.Background(), "hoodi-001", s.URL, f, "positive", now); e == nil {
				t.Fatal("accepted")
			}
		})
	}
	t.Run("negative200", func(t *testing.T) {
		...
		defer s.Close()
		if _, e := runEngineProbeWith(context.Background(), "hoodi-001", s.URL, "/missing", "missing", now); e == nil {
	...
	t.Run("negative500", func(t *testing.T) {
		...
		defer s.Close()
		if _, e := runEngineProbeWith(context.Background(), "hoodi-001", s.URL, "/missing", "wrong", now); e == nil {
	...
	t.Run("malformedkey", func(t *testing.T) {
		...
		if _, e := runEngineProbeWith(context.Background(), "hoodi-001", "http://127.0.0.1:1", f, "positive", now); e == nil {
