...
func TestVaultReadMatrixAndNoBodies(t *testing.T) {
	for _, workload := range []string{"nethermind", "prysm", "signer", "client", "db"} {
		t.Run(workload, func(t *testing.T) {
			p, _ := profile("hoodi-001", workload)
			...
			result, err := checkVaultReadBoundary("hoodi-001", workload, []byte("jwt-secret"), req)
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			req := func(method, path, token string, body []byte) (*http.Response, error) {
				...
				if path == "node-operator-runtime/data/validators/hoodi/hoodi-001/runtime/client-tls" {
					return response(200, ""), nil
				}
				return response(403, ""), nil
			}
			r, e := checkVaultReadBoundary("hoodi-001", "client", []byte("jwt"), req)
			if (e != nil) != tc.wantErr {
				t.Fatalf("err %v", e)
			}
