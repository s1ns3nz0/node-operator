...
func TestProductionConfigBindsDirectRouteToCanonicalTLSAudience(t *testing.T) {
	config := productionConfig("hoodi-001", testPublicKey)
	if config.endpoint != "https://validator-hoodi-001-remote-signer-direct.validator-operations.svc:9000/api/v1/eth2/publicKeys" {
		t.Fatalf("unexpected fixed endpoint: %s", config.endpoint)
	}
	if config.serverName != "validator-hoodi-001-remote-signer.validator-operations.svc" {
