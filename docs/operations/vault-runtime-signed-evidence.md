...
```sh
gh workflow run vault-runtime-candidate-verification.yml --ref main -f sign_evidence=true
```
...
The verifier requires exact workflow identity, issuer, source SHA, repository,
main ref and manual trigger. It then regenerates the statement from all three
