...
| --- | --- | --- | --- |
| `template-injection` | `ci.yml`, `release.yml`, `image-release.yml`, `opa-pr-gate.yml` (current consolidated paths) | `github.actor` was interpolated directly into a shell `run:` block during GHCR login. | Resolved by mapping `github.actor` and `github.token` to step-scoped environment variables, then expanding shell variables only. |
