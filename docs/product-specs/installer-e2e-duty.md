...
Input-boundary clarification approved 2026-09-15: fresh deployment names must avoid minute-resolution collisions while remaining within the naming limit. A withdrawal address is user-owned and must not fall back to a maintainer wallet. Show the selected primary and audit-replica Regions and approved CIDRs before resource mutation. Destination IAM trust configuration must be explicitly bound to the intended GitHub repository and IDs; release artifact signature identities remain independently pinned. Selected destination-resource validators must use the selected Region rather than silently assume Seoul. Reusing existing configuration must not regenerate validator identity or resume resources.
...
- Before local bundle building or deployment confirmation, display numbered manual prerequisites and how to check readiness: AWS login/account permissions, non-secret configuration, custody preparation, and GitHub App/CodeConnections approval for operators running their own CI/release infrastructure. Distinguish that approval from the GitHub OIDC provider and exact IAM trust handled by IaC. Explain GitHub Variables versus Secrets and keep all credentials out of `.env`. Do not require GitHub administration merely to install an already verified release. Guidance must not claim checks have run when it only displays commands.
- Read non-secret defaults from one selected configuration file: repository `.env` or legacy `release/env`; extracted bundles also support `env`. Reject conflicting files instead of silently merging them. Resolve relative work/keystore paths from the original configuration directory, including when the entrypoint copies configuration into a temporary bundle; expand only literal `~/` and `$HOME/` prefixes, never execute shell expressions. Confirm the actual AWS account, Region and deployment identity. Never overwrite GITHUB_TOKEN.
...
- Retain separate, permission-restricted diagnostic logs for each invocation under the selected work directory. Record UTC timestamps, mode, source revision, stage/checkpoint, elapsed time and exit status. Record a failure source/line only when available; never capture raw commands, environment values, ceremony transcripts, tokens, recovery shares or passwords. Local checkout runs default to a durable directory under the operator's home rather than a temporary bundle directory.
...
Successful activation alone is not completion. For the currently authorized local E2E run, observe one assigned epoch after this deployment's explicit activation; the local runner must be explicitly configured for one finalized duty. The distributable installer compatibility default remains three consecutive assigned epochs. Match deployment/release identity, validator public key/index, slot/epoch and attestation data with client/fence/signer activity and canonical chain inclusion. Verify the inclusion is in finalized canonical history using the private Beacon and an independently sourced public chain observation. Merely finalized epochs, signed requests, container health or old validator activity are not proof of this duty.
