...
data "aws_iam_policy_document" "validator_client_ecr_key" {
  ...
  statement {
    principals { type = "Service" identifiers = ["ecr.amazonaws.com"] }
    actions = ["kms:CreateGrant", "kms:Decrypt", "kms:DescribeKey", "kms:Encrypt", "kms:GenerateDataKey*", "kms:ReEncrypt*"]
    resources = ["*"]
    condition { test = "StringEquals" variable = "kms:CallerAccount" values = [var.aws_account_id] }
    condition { test = "StringEquals" variable = "kms:ViaService" values = ["ecr.${var.aws_region}.amazonaws.com"] }
  }
}
...
resource "aws_ecr_repository" "validator_client" {
  ...
  encryption_configuration { encryption_type = "KMS" kms_key = aws_kms_key.validator_client_ecr[0].arn }
...
data "aws_iam_policy_document" "github_validator_client_mirror_assume_role" {
  ...
  statement {
    ...
    principals { type = "Federated" identifiers = ["arn:aws:iam::${var.aws_account_id}:oidc-provider/token.actions.githubusercontent.com"] }
    condition { test = "StringEquals" variable = "token.actions.githubusercontent.com:aud" values = ["sts.amazonaws.com"] }
    condition { test = "StringEquals" variable = "token.actions.githubusercontent.com:sub" values = ["repo:${var.github_repository}:environment:validator-client-ecr-mirror"] }
  }
}
