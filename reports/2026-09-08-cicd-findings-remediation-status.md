...
| H3: deploy supply-chain acceptance | Approved source/digest binding, Cosign signature and provenance, fresh SBOM/SCA; additional digest-to-downloaded-blob binding under review | Integrate final tests, establish genuinely protected approval boundary, execute live private CD acceptance. |
| H4: real private-service DAST | Narrow NetworkPolicies, public certificate-only trust installer, fixed signer GET `/upcheck` proxy; no direct DAST-to-signing API permission | Finish behavioral tests/review; explicitly authorized deployment and live positive/negative connectivity tests. No runtime manifests applied in this checkpoint. |
